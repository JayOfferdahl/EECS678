...
int check_for_deadlock()
{
  ...
  unsigned long new_sys_time = 0;
  unsigned long new_user_time = 0;
  ...
  for (i = 0; i < NUM_PHILS; i++) {
    ...
    if(!statf)
      printf("There was an error opening the file...filename: %s\n", filename);
     ...
     */
    if(fclose(statf))
      printf("There was an error closing the file...filename: %s\n", filename);
